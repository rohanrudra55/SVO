...
cv::Mat Stereo::disparity,Stereo::reprojection_matrix;
double Stereo::focal_length,Stereo::baseline;
std::string Stereo::channel;
void Stereo::disparityCalculate(cv::Mat &image_left, cv::Mat &image_right){
    ...
    sgbm->setNumDisparities(num_diaparities);
...
void Stereo::depthCalculate(cv::Mat& depth_map){
    depth_map=cv::Mat::zeros(disparity.size(),disparity.type());
    if(channel=="single" || channel=="Single"){
        for(int _row=0;_row<(int)disparity.rows;_row++){
            for(int _col=0;_col<(int)disparity.cols;_col++){
                ...
                double depth=0.1;
                if(pixel>0 && baseline>0 && focal_length>0)
                    depth = (focal_length*baseline)/pixel;
                depth_map.at<double>(_row,_col)=depth;
                // std::cout<<_row<<" "<<_col<<"Depth"<<depth<<std::endl;
            }
        }
    }
    else{
        cv::reprojectImageTo3D(disparity,depth_map,reprojection_matrix,true);
    }
}
// int main(){
//     cv::Mat leftimage=cv::imread(" ",0);
//     cv::Mat rightimage=cv::imread(" ",0);
//     cv::Mat disp;
//     Stereo::disparityCalculate(leftimage,rightimage);
//     disp=Stereo::getDisparity();
//     cv::namedWindow("filtered disparity", cv::WINDOW_AUTOSIZE);
//     cv::imshow("filtered disparity", disp);
// }
